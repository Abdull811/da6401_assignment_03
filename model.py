...
class Transformer(nn.Module):
    ...
    def _load_pretrained_artifact(
        ...
        if file_id and not path:
            path = "pretrained_checkpoint.pt"

        if path and not os.path.exists(path) and file_id:
            try:
                import gdown
    ...
    def _beam_decode_ids(
        ...
        for _ in range(max_len - 1):
            ...
            for token_ids, score, finished in beams:
                ...
                logits = self.decode(memory, src_mask, tgt, tgt_mask)[:, -1, :]
    ...
    def infer(self, german_sentence: str) -> str:
        ...
        with torch.no_grad():
            memory = self.encode(src, src_mask)
            generated = self._beam_decode_ids(memory, src_mask, device)
